...
export function Hero({ invite, couple }: HeroProps) {
  ...
  return (
    <header className="relative flex min-h-[100svh] flex-col items-center justify-center overflow-hidden px-6 py-24 text-center">
      ...
      {/* Scrim: keeps text at accessible contrast over any photo */}
      <div
        className="absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_center,rgba(16,12,12,0.55)_0%,rgba(16,12,12,0.82)_60%,rgba(16,12,12,0.96)_100%)]"
        aria-hidden="true"
      />
      <div
        className="absolute inset-x-0 bottom-0 -z-10 h-40 bg-gradient-to-b from-transparent to-ink"
        aria-hidden="true"
      />

      <div className="animate-fade-up flex w-full max-w-2xl flex-col items-center gap-7">
      ...
      {/* Scroll cue */}
      <div className="animate-shimmer absolute bottom-8 flex flex-col items-center gap-2" aria-hidden="true">
